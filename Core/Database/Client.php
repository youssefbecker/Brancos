...
class Client extends Table{
    ...
    public static function save($nom, $numero, $password, $email, $entete, $id=null){
        $sql = 'INSERT INTO ';
        $baseSql = self::getTable().' SET nom = :nom, numero = :numero, password = :password,email = :email,entete = :entete';
        $baseParam = [':nom'=>(ucwords($nom)),':password'=>($password),':email'=>($email),':numero'=>($numero),':entete'=>($entete)];
        if(isset($id)){
            $sql = 'UPDATE ';
            $baseSql .= ' WHERE id = :id';
            $baseParam [':id'] = $id;
